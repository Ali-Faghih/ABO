...
export const BookAppointmentScreen = ({ request, onBack, onChat }: Props) => {
  ...
  const [donor, setDonor] = useState<DonorProfile | null>(user as DonorProfile | null);
  ...
  useEffect(() => {
    if (user?.id) {
      getUserById(user.id).then((fresh) => { if (fresh) { setDonor(fresh as DonorProfile); if (fresh.eligible !== user.eligible) updateProfile(fresh); } });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (selectedTime) {
      ...
      const [h, m] = selectedTime.split(":").map((s) => Number(toAscii(s)));
  ...
  }, [step, selectedDate, selectedTime, request.id, donor]);
  ...
  const renderDoneSheet = () => (
    <>
      <div className="fixed inset-0 bg-black/40 z-40" onClick={onBack} />
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 z-50 bg-white rounded-t-3xl px-6 pt-6 pb-10 shadow-xl animate-slide-up w-full max-w-[430px] flex flex-col items-center gap-5" dir="rtl" style={{ fontFamily: "'Vazirmatn', sans-serif" }}>
        <div className="w-16 h-1 bg-gray-200 rounded-full mb-2" />
        <div className="w-20 h-20 bg-green-50 rounded-full flex items-center justify-center"><CheckCircle size={40} className="text-green-500" /></div>
        <div className="text-center"><h2 className="text-xl font-bold text-foreground mb-1">نوبت شما رزرو شد!</h2>
          <p className="text-xs text-muted-foreground leading-relaxed">نوبت اهدای خون شما در تاریخ {selectedDate} ساعت {selectedTime} در {request.hospitalName} ثبت شد.</p>
        </div>
        {countdown && (
  ...
  return (
    <>
      <div className={`flex flex-col h-full bg-[#F4F6FB] ${step === "done" ? "opacity-30 pointer-events-none" : ""}`} dir="rtl" style={{ fontFamily: "'Vazirmatn', sans-serif" }}>
