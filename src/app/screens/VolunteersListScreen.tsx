...
import { ArrowLeft, User, MessageCircle, Search, MapPin } from "lucide-react";
...
export const VolunteersListScreen = ({ onBack, onChat }: Props) => {
  ...
  const filtered = search
    ? donors.filter((d) => d.donorName.includes(search) || d.bloodType.includes(search) || d.city.includes(search))
    : donors;
        ...
        <div className="mx-4">
          ...
          {filtered.length === 0 ? (
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-border/20 text-center">
              ...
              <p className="text-sm text-muted-foreground">{search ? "نتیجه‌ای یافت نشد." : "هنوز داوطلبی ثبت‌نام نکرده است."}</p>
            </div>
          ) : (
